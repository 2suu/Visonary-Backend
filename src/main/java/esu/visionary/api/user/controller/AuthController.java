...
public class AuthController {
    ...
    public ResponseEntity<?> sendVerificationCode(@RequestBody VerifyRequest request) {
        // 이름, 주민등록번호, 전화번호, 통신사, 성별 받음
        String transactionId = UUID.randomUUID().toString();
        String code = String.valueOf((int)(Math.random() * 900000) + 100000);

        // 인증번호를 transactionId와 함께 저장
        authCodeMap.put(transactionId, code);

        // 서버 로그에 출력 (테스트용)
        System.out.println("🔐 인증번호 전송됨 [" + transactionId + "] → 코드: " + code);
    ...
    public ResponseEntity<?> signup(@RequestBody SignupRequest request) {
        ...
        User user = new User(
                (long) (new Random().nextInt(9000) + 1000), // 1000~9999 사이의 4자리 숫자
                request.getId(),
